...
type board (rows : int, cols : int) =
  ...
  let _fields = Array2D.create rows cols ((false, None) : State)
  ...
  member this.registerShot (coords : int*int, s : ship option) =
    _fields.[fst coords, snd coords] <- (true, s)
  ...
  /// that ship
  member this.shootAt (coords : int*int) : bool =
  ...
  override this.ToString () =
    let digits = max rows cols |> float |> log10 |> ceil |> int
...
type player (name : string) =
  ...
  let _name = name
  // The player's board
  let _myBoard = board(10, 10)
  // The player's knowledge about the opponent's board
  let _opponentsBoard = board(10, 10)
  // The player's list of ships
  let _ships = List.map (fun elm -> ship(elm)) [2;2;3;5]
...
type game() =
  ...
  let _p0 = player("Player 1")
  let _p1 = player("Player 2")
