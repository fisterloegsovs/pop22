...
\newcommand{\handinTextPdfTexFsx}[3]{Afleveringen skal bestå af
\begin{itemize}
  \item en zip-fil, der hedder \texttt{#1.zip}
  \item en rapport i pdf-format.
\end{itemize}
Zip-filen skal indeholde en \texttt{src} mappe, en \texttt{tex} mappe
og filen \texttt{README.txt}. \texttt{README.txt} filen skal indeholde
jeres navn og dato for arbejdet. I \texttt{src} mappen skal der ligge
følgende og kun følgende filer:
\begin{quote}
#2
\end{quote}
svarende til afleveringsopgaverne beskrevet ovenfor. I \texttt{tex}
mappen skal der ligge følgende og kun følgende filer:
\begin{quote}
#3
\end{quote}
Rapporten skal dokumentere jeres proces og beskrive jeres
programmer. Den skal skrives i \LaTeX, indeholde et passende antal
figurer og henvise til jeres afleveringsprogrammer.}
...
fsharpc, og de oversatte filer skal kunne køres med dotnet. Mappen
