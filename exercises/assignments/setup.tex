...
\begin{itemize}\item upload af de uviklede programmer i klassens studio
...
af \begin{itemize}\item en zip-fil og \item en rapport i
pdf-format.\end{itemize} Zip-filen skal indeholde mapperne
\texttt{tex} og \texttt{src} og filen \texttt{README.txt}. Mapperne skal
indeholde henholdsvis LaTeX koden m.m.\ og fsharp koden. Der skal være
en fsharp tekstfil per fsharp-opgave, og de skal navngives \texttt{#1}
osv. Rapporten skal kunne oversættes vha.\ \texttt{pdflatex}
kommandoen i \texttt{tex}-mappen, og fsharp koden skal kunne
oversættes med fsharpc og køres med mono i \texttt{src} mappen. Filen
\texttt{README.txt} skal ganske kort beskrive, hvordan rapporten
oversættes til pdf og koden køres.}

\newcommand{\handinTextThree}[1]{Afleveringen skal bestå af en
zip-fil, som skal indeholde en mappe med fsharp koden. Der skal være
en fsharp tekstfil per fsharp-opgave, og de skal navngives \texttt{#1}
osv. De skal kunne oversættes med fsharpc og køres med
...
kommentarer i de fsx-filer, de hører til.}
