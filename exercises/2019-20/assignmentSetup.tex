...
\newcommand{\handinTextThreeTwo}[3]{%
  ...
  \end{itemize}
  Zip-filen \texttt{#1\_<navn>.zip} skal indeholde en og kun en mappe \texttt{#1\_<navn>}. I den mappe skal der ligge en \texttt{src} mappe og filen \texttt{README.txt}.  I \texttt{src} skal der ligge følgende og kun følgende filer: #2 svarende til hver af delopgaverne.  De skal kunne oversættes med \lstinline[language=console]{fsharpc}, og de oversattte filer skal kunne køres med mono.  Funktioner skal dokumenteres ifølge dokumentationsstandarden som minimum ved brug af \texttt{<summary>}, \texttt{<param>} og \texttt{<returns>} XML-tagsne. Filen \texttt{README.txt} skal ganske kort beskrive, hvordan koden oversættes og køres. %
