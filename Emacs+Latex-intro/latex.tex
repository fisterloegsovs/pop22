...
kryptiske. En beskrivelse af de hyppigst forekomne fejlmeddelelser, og
...
producere PDF formatteret til A4-papir, ændrer vi første linje til
...
kommandoer, og derefte lave forsiden med kommandoen
\verb|\maketitle|.  Titelsiden på indeværende
dokument er lavet med kommandoerne

{\small
\begin{verbatim}
\title{En superkort introduktion til Emacs og \LaTeX}
\author{Torben Mogensen}
\date{\today}
\maketitle
\end{verbatim}
...
En rapport består af kapitler, der er opdelt i nummererede afsnit og
underafsnit.  \LaTeX\ finder selv ud af nummereringen, man skal blot
angive titlerne på kapitler og afsnit.  I rapporter starter kapitler
på nye sider, så \LaTeX-dokumentet vist i figur~\ref{simple} vil
generere to korte kapitler startende på hver sin side.  Prøv selv at
kopiere teksten ind i Emacs, gem den under navnet \texttt{simple.tex},
og kør kommandoen \texttt{pdflatex~simple.tex}.
...
ikke en separat titelside, titel osv.\ placeres blot øverst på første
...
Når man vil vise programtekster, er det irriterende, at \LaTeX
ignorerer linje\-skift og gentagne blanktegn.  Derfor bruger man
\texttt{verbatim} omgivelserne.  Omgivelser er en særlig slags
kommandoer, der typisk bruges til ting, der står på linjer for sig
selv i stedet for midt i en tekst.  I stedet for at skrive en enkelt
kommando med et argument i krølleparenteser, bruger man kommandoen
\verb|\begin| til at starte omgivelsen og \verb|\end| til at afslutte
den.  Vi har allerede set \verb|\begin{document}| og
...
Der findes pakker til \LaTeX, der kan lave mere avanceret formattering
...
\noindent
Dette gør, at jeg ved at skrive \verb|afsnit~\ref{supplerendeLatex}| får
krydsreferencen ``afsnit~\ref{supplerendeLatex}''.  Hvis den genererede PDF
fil læses på en skærm i stedet for papir, vil krydsreferencen
endvidere være et link, man kan følge med et museklik.  Hvis man
bruger \verb|\ref| med et navn, der ikke er defineret med en
\verb|\label|-kommando, skriver \LaTeX\ \textbf{??} i stedet for
krydsreferencen, og \texttt{pdflatex} skriver
...
\noindent
Formler inde i tekstafnit omgives af dollartegn, så den viste formel
...
\item Blanktegn lige efter en kommando, der ikke har parametre i
  ...
  \verb|\LaTeX styrer!| give resultatet {\LaTeX styrer!}.  For at få
  et blanktegn ind alligevel give kommandoen en tom parameter, altså
  \verb|\LaTeX{} styrer!|, som giver resultatet {\LaTeX\ styrer!}.  Det
  er ikke nødvendigt, hvis kommandoen følges af andet end blanktegn
  eller linjeskift, som f.eks.\ punktum eller komma.  Alternativt kan
  man skrive \verb|\LaTeX\ styrer!|, altså sætte et backslash før
  blanktegnet, så det ikke bliver undertrykt.
...
\item \LaTeX\ laver lidt større mellemrum efter et punktum, der
  ...
  Hvis kommaet står lige efter et stort bogstav, tror \LaTeX, at der
...
Der findes en meget omfattende bog om \LaTeX\ på dansk, skevet af Leif
