...
let dk = wb.Countries.``Denmark``.Indicators
...
let dkschool = series dk.``School enrollment, tertiary (% gross)``
...
// DK is missing 1997, which causes problems when plotting
let startYear = 1998;
let endYear = 2013
[ dkschool.[startYear .. endYear]; czschool.[startYear .. endYear]; euschool.[startYear .. endYear] ]
|> Chart.Line
|> Chart.WithOptions (Options(legend=Legend(position="bottom")))
|> Chart.WithLabels ["DK"; "CZ"; "EU"]
