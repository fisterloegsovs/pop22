...
\date{25.\ oktober - 1.\ november.\\Afleveringsfrist: onsdag d.\ 1.\ november kl. 22:00}
...
\item Med udgangspunkt i øvelsesopgave~\ref{sierpinski.ov} skal du i
  ...
  version af X-fractalen som illustreret til højre.

  Dybden for fraktalen skal kunne styres med piletasterne som det er
  tilfældet med Sierpinski-fraktalen i
  øvelsesopgave~\ref{sierpinskikeys.ov}.
\end{enumerate}
...
\item Du skal i de følgende to opgaver arbejde med en funktion til at
  ...
  \item Den direkte implementerede rekursive funktion er temmelig
    ...
    sig af $O(|a| \times |b|)$ plads og at der findes løsninger der
    benytter sig af mindre plads ($O(\mathrm{max}(|a|,|b|))$).
\end{enumerate}

Afleveringsopgaven skal afleveres som et antal \texttt{fsx} tekstfiler
navngivet efter opgaven, som f.eks. \lstinline!5i0.fsx!. Tekstfilerne
